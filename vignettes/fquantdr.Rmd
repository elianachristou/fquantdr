...
## ADHD-200 fMRI data
To illustrate the `fcqs` function, we use the ADHD-200 fMRI data to visualize our output. First, we load the data for the response and predictor variables.

```{r}
# Loading the Data
load("/Users/a33/Dropbox/Shanshan/rcode/categorical/real data/fmri.data.com.Rdata")
dim(fmri.data.com)
...
#response is the ADHD index for combined
NYU_phenotypic=read.csv("~/Dropbox/Shanshan/rcode/categorical/real data/NYU_phenotypic.csv", sep=";")
head(NYU_phenotypic)
missing.data <- which(NYU_phenotypic[,"ADHD.Index"] == -999 | NYU_phenotypic[,"QC_Rest_1"] == "N/A") # 10 missing data 
# remove the missing data
NYU_phenotypic=NYU_phenotypic[-c(missing.data),] 
nrow(NYU_phenotypic) 
n=length(which(NYU_phenotypic[,"DX"]==1)) #number of combined
y=NYU_phenotypic[,"ADHD.Index"][which(NYU_phenotypic[,"DX"]==1)]
```
...
# output2 <- fcqs(Xc, y, tt, nbasis, tau = 0.5, d_tau = 5) # error in y (72) should be greater than the number of predictors (116)
```

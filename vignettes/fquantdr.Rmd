...
```{r}
output1 <- fcqs(Xc, y, time_points, nbasis, tau, d_tau)
output1
```
The output of the `fcqs` function includes two primary components:
1. `betacoef`: The functional parameters that span the FCQS. These are the estimated directions in the functional central quantile subspace, which reflect the significant functional predictors for the quantile regression.

1. `betax`: The resulting sufficient predictor, calculated as the inner product between `betacoef` and `x`. This represents the reduced-dimensional representation of the functional data that retains the essential information for the quantile regression.
...
```{r}
# Plotting the first estimated direction in the FCQS
plot(time_points, output1$ffun[, 1], type = 'l', col = 'blue',
     ylab = 'Estimated direction', xlab = 'Time', 
     main = 'Estimated FCQS Direction')
```
